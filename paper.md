...
-   **Valley Floor Delineation – Sechu** implements a cost-accumulation method (@sechu_2021) to identify low-relief valley-floor areas from a DEM, serving as a starting point for defining valley boundaries.
...
-   **[1] Generate Transects**: Perpendicular transects are generated from each river-segment centerline to the valley boundaries. The tool iteratively extends transects until intersections with both left and right valley-boundary lines are found, providing a consistent sampling framework for valley geometry.
...
-   **[4] Extract LVS, RVS, and MVS**: Left and Right Valley Slope (LVS, RVS) are computed using elevation differences between valley-bottom and valley-top intersections along each transect. Mean Valley Slope (MVS) averages the two and provides an index of valley asymmetry.

-   **[5] Extract DVS and SIN**: Down-Valley Slope (DVS) is calculated from segment endpoints in the stream network. River Sinuosity (SIN) compares segment length to its straight-line distance.
